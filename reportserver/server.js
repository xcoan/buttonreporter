...
app.get('/', (req, res) => {
  db.collection('crimes').find().toArray((err, result) => {
    if (err) return console.log(err)
    res.render('index.ejs', {crimestamps: result})
  })
})

// Creates new quote document with given name and quote
app.post('/crime/:source', (req, res) => {
  // Current time for timestamp
  var date = new Date()
  var src = req.params.source
  db.collection('crimes').save({ timestamp: date.toISOString(), source: src}, (err, result) => {
    if (err) return console.log(err)
